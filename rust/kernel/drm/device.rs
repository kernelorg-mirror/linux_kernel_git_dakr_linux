...
impl<T: drm::Driver> UnregisteredDevice<T> {
    ...
    pub fn new(dev: &T::ParentDevice<device::Bound>) -> Result<Self> {
...
pub struct Device<T: drm::Driver, C: DeviceContext = Registered> {
    ...
    pub(super) data_is_init: Atomic<bool>,
    ...
    T::Data: HasWork<Device<T>, ID>,
{
    unsafe fn raw_get_work(ptr: *mut Self) -> *mut Work<Device<T>, ID> {
        ...
        // CAST: `UnsafeCell` and `MaybeUninit` are both `#[repr(transparent)]`.
        let data_ptr: *mut T::Data = unsafe { &raw mut (*ptr).data }.cast();
    ...
    unsafe fn work_container_of(ptr: *mut Work<Device<T>, ID>) -> *mut Self {
        ...
        // CAST: `UnsafeCell` and `MaybeUninit` are both `#[repr(transparent)]`.
        let data_ptr: *mut UnsafeCell<MaybeUninit<T::Data>> = data_ptr.cast();
